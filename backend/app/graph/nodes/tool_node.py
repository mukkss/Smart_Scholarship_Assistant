    ...
    for call in last_message.tool_calls:
        ...
        if name == "scholarship_tool":
            ...
            if "no results" in rag_result.lower() or "not found" in rag_result.lower():
                web_query = args.get("query", "scholarships")
                web_result = google_search_tool.invoke(web_query) 
                
                results.append(ToolMessage(
                    content=f"RAG: No results\nWeb: {web_result}",
                    tool_call_id=call["id"]
                ))
            else:
                results.append(ToolMessage(content=rag_result, tool_call_id=call["id"]))

        elif name == "google_search_tool":
            web_query = args.get("query", "")
            web_result = google_search_tool.invoke(web_query)
            results.append(ToolMessage(content=web_result, tool_call_id=call["id"]))
